...
const EditProfile = ({ resp }) => {
    ...
    const handleProfile = () => {
        ...
        if (data.name && data.email && data.number && data.username) {
            if (name.borderBottom === errStyle.borderBottom || email.borderBottom === errStyle.borderBottom || number.borderBottom === errStyle.borderBottom || username.borderBottom === errStyle.borderBottom) toast.error('Enter valid data', {
                position: "top-center",
                autoClose: 3000,
                hideProgressBar: false,
                closeOnClick: true,
    ...
    const handlePassword = () => {
        ...
        if (curpass && newpass && cnewpass) {
            if (validation.curpass.borderBottom === errStyle.borderBottom || validation.newpass.borderBottom === errStyle.borderBottom) toast.error('Enter valid data', {
                position: "top-center",
                autoClose: 3000,
                hideProgressBar: false,
                closeOnClick: true,
            ...
            else if (newpass !== cnewpass) toast.error('Password not matching', {
                position: "top-center",
                autoClose: 3000,
                hideProgressBar: false,
                closeOnClick: true,
    ...
    const formValidation = (name, value) => {
        if (name === 'name') {
            ...
            if (data2.newpass === value) setValidation({ ...validation, [name]: { ...styleVal.successStyle } });
            else setValidation({ ...validation, [name]: { ...styleVal.errStyle } });
        }
    }
        ...
        <>
                ...
                <div className="edit-profile">
                    ...
                    <div className="edit-data-form">
                        <div className="input-container">
                            ...
                            <input type="text" placeholder='Enter your name' name='name' value={data.name} onChange={handleInput} style={validation.name} required />
                        </div>
                        <div className="input-container">
                            <label>Edit Number</label>
                            <input type="text" placeholder='Edit your number' name='number' value={data.number} onChange={handleInput} style={validation.number} required />
                        </div>
                        <div className="input-container">
                            <label>Edit Username</label>
                            <input type="text" placeholder='Edit username' name='username' value={data.username} onChange={handleInput} style={validation.username} required />
                        </div>
                        <div className="input-container">
                            <label>Edit Email</label>
                            <input type="text" placeholder='Edit your email' name='email' value={data.email} onChange={handleInput} style={validation.email} required />
                        </div>
                    </div>
                    <div className="radio-container">
                        <label>Edit Gender : </label>
                        {item2.map((value, indx) => <div key={indx}>
                            <input type="radio" name="gender" id={value.id} value={value.val} checked={value.check} onChange={value.inputData} />
            ...
            <div className="edit-profile-container pass-profile-container">
                ...
                <div className="edit-password">
                    ...
                    <input type="password" name="newpass" value={data2.newpass} onChange={handleInputPassword} style={validation.newpass} placeholder='Enter new password' />

                    <label>Confirm Password</label>
                    <input type="password" name="cnewpass" value={data2.cnewpass} onChange={handleInputPassword} style={validation.cnewpass} placeholder='Confirm password' />
                </div>
                <div className="save-first-btn-box">
